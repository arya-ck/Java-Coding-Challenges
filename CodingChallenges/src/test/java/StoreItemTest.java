...
import com.challenge.eoysale.StoreItem;
...
public class StoreItemTest {
    ...
    public void getSetName() {
        StoreItem recordPlayerItem =
                new StoreItem("Record Player",
                        108.50, .65);
        assertEquals("Record Player",
                recordPlayerItem.getName());
        recordPlayerItem.setName("Pink Record Player");
        assertEquals("Pink Record Player",
                recordPlayerItem.getName());
    }
    ...
    public void getSetRetailPrice() {
        StoreItem recordPlayerItem =
                new StoreItem("Record Player",
                        108.50, .65);
        assertEquals(108.50,
                recordPlayerItem.getRetailPrice());
    ...
    public void toStringy() {
        StoreItem recordPlayerItem =
                new StoreItem("Record Player",
                        108.50, .65);
        assertEquals("Name: Record Player, " +
                "Retail price: 108.5, Discount 0.65",
                recordPlayerItem.toString());
    }

    @Test
    public void findLeastExpensive() {
        StoreItem leastExpensive =
                new StoreItem("Dress",
                        34.99, .75);
    ...
    public void findLeastExpensive_sameItemsDifferentDiscounts() {
        StoreItem leastExpensive =
                new StoreItem("T-Shirt",
                        19.99, .9);
        StoreItem secondLeastExpensive =
                new StoreItem("T-Shirt",
                        19.99, .7);
        StoreItem thirdLeastExpensive =
                new StoreItem("T-Shirt",
                        19.99, .6);
